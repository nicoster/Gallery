...
import com.ocp.app.App;
import com.ocp.picasa.AlbumEntry;
import com.ocp.picasa.Entry;
import com.ocp.picasa.EntrySchema;
import com.ocp.picasa.PhotoEntry;
import com.ocp.picasa.PicasaApi;
import com.ocp.picasa.PicasaContentProvider;
import com.ocp.picasa.PicasaService;
...
public final class PicasaDataSource implements DataSource {
    ...
    private static final String DEFAULT_BUCKET_SORT_ORDER = AlbumEntry.Columns.USER + ", "
            + AlbumEntry.Columns.DATE_PUBLISHED + " DESC";
    ...
    public void loadMediaSets(final MediaFeed feed) {
        ...
        if (mProviderClient == null) {
            mProviderClient = mContext.getContentResolver().acquireContentProviderClient(
                    PicasaContentProvider.AUTHORITY);
        }
    ...
    protected void loadMediaSetsIntoFeed(final MediaFeed feed, boolean sync) {
        ...
        try {
            ...
            final Cursor cursor = client.query(PicasaContentProvider.ALBUMS_URI, albumSchema.getProjection(), null,
                    null, DEFAULT_BUCKET_SORT_ORDER);
            ...
            if (cursor.moveToFirst()) {
                ...
                do {
                    ...
                    final boolean accountEnabled = (accountEnabledObj == null) ? false : accountEnabledObj
                            .booleanValue();
                    if (accountEnabled) {
            ...
            Log.e(Gallery.TAG, TAG + ": " + "Error occurred loading albums");
        }
    }
    ...
    private void addItemsToFeed(MediaFeed feed, MediaSet set, int start, int end) {
        ...
        try {
            ...
            cursor = client.query(PicasaContentProvider.PHOTOS_URI, photosSchema.getProjection(), whereInAlbum, null,
                    null);
            ...
            Log.e(Gallery.TAG, TAG + ": " + "Error occurred loading photos for album " + set.mId);
        } finally {
            if (cursor != null) {
                cursor.close();
            }
    ...
    public void refresh(final MediaFeed feed, final String[] databaseUris) {
        // Depending on what URI changed, we either need to update the mediasets
        // or the mediaitems of a set.
        if (databaseUris != null && databaseUris.length > 0) {
            if (ArrayUtils.contains(databaseUris, PicasaContentProvider.ALBUMS_URI.toString())) {
                ...
                // This operation is not yet supported, so we might as well
                // refresh everything.
                final ArrayList<MediaSet> mediaSets = feed.getMediaSets();
                final int numMediaSets = mediaSets.size();
                for (int i = 0; i < numMediaSets; ++i) {
