...
public final class PicasaContentProvider extends TableContentProvider {
    public static final String AUTHORITY = "com.ocp.picasa.contentprovider";
    ...
    public void attachInfo(Context context, ProviderInfo info) {
        ...
        addMapping(AUTHORITY, "photos", "vnd.ocp.picasa.photo", PhotoEntry.SCHEMA);
        addMapping(AUTHORITY, "albums", "vnd.ocp.picasa.album", AlbumEntry.SCHEMA);

        // Create the sync context.
        try {
            mSyncContext = new SyncContext();
        } catch (Exception e) {
            // The database wasn't created successfully, we create a memory
            // backed database.
    ...
    private UserEntry[] syncUsers(SyncContext context, SyncResult syncResult) {
        ...
        if (cursor.moveToFirst()) {
            do {
                ...
                if (i == numUsers) {
                    Log.e(Gallery.TAG, TAG + ": " + "Deleting user " + entry.account);
                    entry.albumsEtag = null;
                    deleteUser(db, entry.account);
                }
            } while (cursor.moveToNext());
        } else {
            // Log.i(Gallery.TAG, TAG + ": " + "No users in database yet");
        }
        cursor.close();
        ...
        for (int i = 0; i != numUsers; ++i) {
            ...
            if (entry == null) {
                ...
                users[i] = entry;
                Log.e(Gallery.TAG, TAG + ": " + "Inserting user " + entry.account);
            }
        }
        return users;
    }
    ...
    private void syncUserAlbums(final SyncContext context, final UserEntry user, final SyncResult syncResult) {
        ...
        int result = context.api.getAlbums(accountManager, syncResult, user, new GDataParser.EntryHandler() {
            public void handleEntry(Entry entry) {
                ...
                if (metadata == null || metadata.dateEdited < album.dateEdited) {
                    // Insert / update.
                    Log.i(Gallery.TAG, TAG + ": " + "insert / update album " + album.title);
                    ...
                    // Up-to-date.
                    // Log.i(Gallery.TAG, TAG + ": " + "up-to-date album " +
                    // album.title);
                }
        ...
        for (int i = 0; i != localCount; ++i) {
            ...
            if (!metadata.survived) {
                ...
                Log.i(Gallery.TAG, TAG + ": " + "delete album " + metadata.id);
            }
        }
    ...
    private void syncUserPhotos(SyncContext context, String account, SyncResult syncResult) {
        ...
        SQLiteDatabase db = context.db;
        Cursor cursor = db.query(AlbumEntry.SCHEMA.getTableName(), Entry.ID_PROJECTION,
                "sync_account=? AND photos_dirty=1", new String[] { account }, null, null, null);
        AlbumEntry album = new AlbumEntry();
        for (int i = 0, count = cursor.getCount(); i != count; ++i) {
            ...
            if (Thread.interrupted()) {
                ...
                Log.e(Gallery.TAG, TAG + ": " + "syncUserPhotos interrupted");
            }
        }
        cursor.close();
    }

    private void syncAlbumPhotos(SyncContext context, final String account, AlbumEntry album,
            final SyncResult syncResult) {
        Log.i(Gallery.TAG, TAG + ": " + "Syncing Picasa album: " + album.title);
        // Query existing album entry (id, dateEdited) sorted by ID.
        final SQLiteDatabase db = context.db;
        long albumId = album.id;
        String[] albumIdArgs = { Long.toString(albumId) };
        Cursor cursor = db.query(PhotoEntry.SCHEMA.getTableName(), ID_EDITED_INDEX_PROJECTION, WHERE_ALBUM_ID,
                albumIdArgs, null, null, "date_edited");
        int localCount = cursor.getCount();
        ...
        int result = context.api.getAlbumPhotos(accountManager, syncResult, album, new GDataParser.EntryHandler() {
            public void handleEntry(Entry entry) {
                ...
                if (metadata == null || metadata.dateEdited < photo.dateEdited
                    ...
                    // Insert / update.
                    // Log.i(Gallery.TAG, TAG + ": " + "insert / update photo "
                    // + photo.title);
                    ...
                    // Up-to-date.
                    // Log.i(Gallery.TAG, TAG + ": " + "up-to-date photo " +
                    // photo.title);
                }
        ...
        case PicasaApi.RESULT_ERROR:
            ++syncResult.stats.numParseExceptions;
            Log.e(Gallery.TAG, TAG + ": " + "syncAlbumPhotos error");
        case PicasaApi.RESULT_NOT_MODIFIED:
            // Log.e(Gallery.TAG, TAG + ": " + "result not modified");
            return;
        }
        ...
        for (int i = 0; i != localCount; ++i) {
            ...
            if (!metadata.survived) {
                ...
                ++syncResult.stats.numDeletes;
                // Log.i(Gallery.TAG, TAG + ": " + "delete photo " +
                // metadata.id);
            }
        }
        ...
        // Log.i(Gallery.TAG, TAG + ": " + "Clearing dirty bit on album " +
        // albumId);
    ...
    private void deleteUser(SQLiteDatabase db, String account) {
        Log.w(Gallery.TAG, TAG + ": " + "deleteUser(" + account + ")");

        // Select albums owned by the user.
        String albumTableName = AlbumEntry.SCHEMA.getTableName();
        String[] whereArgs = { account };
        Cursor cursor = db.query(AlbumEntry.SCHEMA.getTableName(), Entry.ID_PROJECTION, WHERE_ACCOUNT, whereArgs, null,
                null, null);
    ...
    private void deleteAlbumPhotos(SQLiteDatabase db, long albumId) {
        Log.v(Gallery.TAG, TAG + ": " + "deleteAlbumPhotos(" + albumId + ")");
